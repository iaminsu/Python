...
path_data = "F:\\data\\Netherland\\GDB4BP-SLOT.gdb\\"
...
p = 10    #number of refueling stations required 
...
while len(solution_stations.solution_sites) < p:
    ...
    if OD_routes_sites[target][0][1] < driving_range/4:
        ...
        for site in candidates:
            ...
            arcpy.SelectLayerByAttribute_management(node_lyr, 'NEW_SELECTION', '"' + nodeID + '" = '+ str(site))
            arcpy.SelectLayerByLocation_management(route_lyr, "INTERSECT", node_lyr)
            for row in arcpy.da.SearchCursor(route_lyr, ["OID@", origin_field, destination_field]):
        ...
        if covered_flow[0][0] == 0:
            ...
            fully_covered.append(target)
            arcpy.SelectLayerByAttribute_management(node_lyr, 'NEW_SELECTION', '"' + nodeID + '" = '+ str(covered_flow[0][1]))
            arcpy.SelectLayerByLocation_management(route_lyr, "INTERSECT", node_lyr)
